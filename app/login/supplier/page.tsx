...
export default function SupplierPortal() {
  ...
  const handleCheckApprovalStatus = async () => {
    ...
    try {
      ...
      if (profile) {
        sessionStorage.setItem('smebhawan_user_session', JSON.stringify(profile))
        setSession(profile)
        if (profile.status === 'verified') {
          showToast('Congratulations! Your account has been verified.', 'success')
        } else {
          showToast('Account is still pending audit verification.', 'info')
        }
      } else {
        showToast('Profile not found in database.', 'error')
      }
    } catch (err) {
      console.error(err)
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-xl backdrop-blur-md text-center space-y-6">
              ...
              <div className="pt-4 border-t border-white/10 flex gap-3">
                ...
                <button
                  ...
                  className="w-full border border-white/10 text-muted-foreground py-2.5 rounded-xl text-xs font-bold transition-transform hover:-translate-y-0.5 hover:text-foreground"
                >
                  Log Out
                </button>
              </div>
