...
export function useAuthGuard(expectedRole: 'customer' | 'supplier' | 'admin') {
  ...
  useEffect(() => {
    ...
    if (expectedRole === 'admin') {
      const isLoggedIn = localStorage.getItem('smebhawan_admin_logged') === 'true'
      if (!isLoggedIn) {
    ...
    const sessionStr = localStorage.getItem('smebhawan_user_session')
    if (!sessionStr) {
      ...
      if (expectedRole === 'customer') {
        ...
        // We will manage it internally in page.tsx itself using the local storage guard!
      }
      return
    }
